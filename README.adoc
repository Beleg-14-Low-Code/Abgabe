...
image:magic_Quadrant.png[,500 "Magic Quadrant for Enterprise Low-Code Application Platforms"]
...
Der Magic Quadrant ist in vier Teile unterteilt und zeigt die Wettbewerbsposition von vier Arten von Technologieanbietern in Märkten mit hohem Wachstum und signifikanter Anbieterdifferenzierung:
...
Zunächst entscheidet der Entwickler über die Architektur der Anwendung. Und je nach Architektur ist es möglich, über zusätzlichen Entwicklungsaufwand zu sprechen. In diesem Beispiel wurde eine Client-Server-Architektur verwendet.
...
Die Wiederverwendung von Code oder Komponenten, die neu entwickelt wurden. 
